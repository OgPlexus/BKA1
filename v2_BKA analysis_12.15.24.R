############################################################
##BKA analysis 
##written by Andrea Ayala last change 12/15/24
##Reference sites:
...
#boxplot(Lead_Imputation ~ Age,data=bka3, main="Pb by Age",
#        xlab="Age", ylab="Pb ug/dl")

p1<-ggplot(data = bka3, aes(x = Age, y = Lead_Imputation)) +
  geom_boxplot() +
  labs(x ='Age', y='Pb ug/dl')

p1 + theme_classic(base_size = 22) # the default
...
#boxplot(Lead_Imputation ~ Sex,data=bka3, main="Pb by Sex",
#        xlab="Sex", ylab="Pb ug/dl")


p2<-ggplot(data = bka3, aes(x = Sex, y = Lead_Imputation)) +
  geom_boxplot() +
  labs(x ='Sex', y='Pb ug/dl')

p2 + theme_classic(base_size = 22) # the default



#ggplot(data=myfile, aes(x=group, y=time) +
         
#         geom_boxplot() +
#         
#         labs(title = "My graph", x = "Groups", y = "Time") +
#         
#         theme(
...
#Plotting glm.bka using the sjPlot library function

library(ggplot2)
library(dplyr) #https://stackoverflow.com/questions/63227849/ggplot2-4-categorical-and-1-continuous-variable

labs = c(expression(AHY),expression(HY))

library(magrittr)

p4<- bka3 %>% 
  mutate(series = interaction(Age)) %>% 
  ggplot() + 
  geom_line(aes(Lead_Imputation, Mean_BKA, color = series, linetype = series), stat="identity") + 
  ...
  ylab("Mean BKA Capacity") +
  xlab("Pb ug/dl")

p4 + theme_classic(base_size = 22) # the default
...
p3<-ggplot(data = bka3, aes(x = Mean_BKA)) +
  geom_histogram(bins=30) +
  theme(axis.text = element_text(size = 100, angle=90)) +
labs(x ='BKA Capacity', y='Number of Mottled Ducks')

p3 + theme_classic(base_size = 22) # the default
